...
class AABBox2 {
 public:
  ...
  bool isInfinite() const {
    if (mMin.x() != std::numeric_limits<T>::lowest() ||
        mMax.x() != std::numeric_limits<T>::max() ||
        mMin.y() != std::numeric_limits<T>::lowest() ||
        mMax.y() != std::numeric_limits<T>::max())
      return false;
    return true;
  }
  void setNull() {
  ...
  void setMinCorner(T x, T y) {
    mMin.x() = x;
    mMin.y() = y;
  }
  void setMinCorner(const Vector2<T>& v) { mMin = v; }
  void setMaxCorner(T x, T y, T z) {
    mMax.x() = x;
    mMax.y() = y;
  }
  ...
  AABBox2& addPoint(const Vector2<T>& v, T radius) {
    ...
    if (mMax.x() < v.x() + radius)
      mMax.x() = v.x() + radius;
    if (mMax.y() < v.y() + radius)
      mMax.y() = v.y() + radius;
    if (mMin.x() > v.x() - radius)
      mMin.x() = v.x() - radius;
    if (mMin.y() > v.y() - radius)
      mMin.y() = v.y() - radius;
    return *this;
  }
  //加入给定点扩展包围盒
  ...
  //判断两包围盒是否不等
  bool operator!=(const AABBox2& box) const { return !operator==(box); }
  //包围盒大小
  ...
  //包围盒的容积
  T volume() const { return width() * height(); }
