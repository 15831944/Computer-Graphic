...
ON_COMMAND(ID_REGULAR_POLYGON, &CMainFrame::OnRegularPolygon)
ON_UPDATE_COMMAND_UI(ID_REGULAR_POLYGON, &CMainFrame::OnUpdateRegularPolygon)
...
ON_UPDATE_COMMAND_UI(ID_VIEW_PROPERTIESWND,
                     &CMainFrame::OnUpdateViewPropertieswnd)
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct) {
  ...
  mdiTabParams.m_bActiveTabCloseButton =
      TRUE;  // 设置为 FALSE 会将关闭按钮放置在选项卡区域的右侧
  mdiTabParams.m_bTabIcons =
      FALSE;  // 设置为 TRUE 将在 MDI 选项卡上启用文档图标
  ...
  m_wndStatusBar.AddElement(
      new CMFCRibbonStatusBarPane(ID_STATUSBAR_PANE1, strTitlePane1, TRUE),
      strTitlePane1);
  m_wndStatusBar.AddExtendedElement(
      new CMFCRibbonStatusBarPane(ID_STATUSBAR_PANE2, strTitlePane2, TRUE),
      strTitlePane2);
...
BOOL CMainFrame::CreateDockingWindows() {
  ...
  m_wndClassView.SetWindowTextW(strClassView);
  return TRUE;
}
...
//图层属性
int CMainFrame::LayerIndex() const {
  CMFCRibbonComboBox* pLayerIndex =
      (CMFCRibbonComboBox*)m_wndRibbonBar.FindByID(ID_COMBO_LAYER);
  if (pLayerIndex)
    return pLayerIndex->GetCurSel();
  return 0;
}
//画笔属性
...
int CMainFrame::PenWidth() const {
  CMFCRibbonEdit* pPenWidth =
      (CMFCRibbonEdit*)m_wndRibbonBar.FindByID(ID_PEN_WIDTH);
  if (pPenWidth)
    return _ttoi(pPenWidth->GetEditText());
  return 1;
}
int CMainFrame::PenStyle() const {
  CMFCRibbonComboBox* pPenStyle =
      (CMFCRibbonComboBox*)m_wndRibbonBar.FindByID(ID_PEN_STYLE);
  if (pPenStyle)
...
bool CMainFrame::UseBrush() const {
  CMFCRibbonCheckBox* pUseBrush =
      (CMFCRibbonCheckBox*)m_wndRibbonBar.FindByID(ID_BRUSH_USE);
  if (pUseBrush)
...
int CMainFrame::BrushIndex() const {
  CMFCRibbonComboBox* pBrushIndex =
      (CMFCRibbonComboBox*)m_wndRibbonBar.FindByID(ID_BRUSH_INDEX);
  if (pBrushIndex)
...
int CMainFrame::LineAlgorithm() const {
  CMFCRibbonComboBox* pLineAlgorithm =
      (CMFCRibbonComboBox*)m_wndRibbonBar.FindByID(ID_COMBO_LINE_ALGORITHM);
  if (pLineAlgorithm)
...
int CMainFrame::CircleAlgorithm() const {
  CMFCRibbonComboBox* pCircleAlgorithm =
      (CMFCRibbonComboBox*)m_wndRibbonBar.FindByID(ID_COMBO_CIRCLE_ALGORITHM);
  if (pCircleAlgorithm)
...
void CMainFrame::OnRegularPolygon() {
  // TODO: 在此添加命令处理程序代码
  mRegularPolygon = !mRegularPolygon;
}

void CMainFrame::OnUpdateRegularPolygon(CCmdUI* pCmdUI) {
  // TODO: 在此添加命令更新用户界面处理程序代码
  pCmdUI->SetCheck(mRegularPolygon);
}
//填充算法
int CMainFrame::FillAlgorithm() const {
  CMFCRibbonComboBox* pFillAlgorithm =
      (CMFCRibbonComboBox*)m_wndRibbonBar.FindByID(ID_COMBO_FILL_ALGORITHM);
  if (pFillAlgorithm)
    return pFillAlgorithm->GetCurSel();
  return 0;
}

int CMainFrame::TimeChose() const {
  CMFCRibbonComboBox* pTimeChose =
      (CMFCRibbonComboBox*)m_wndRibbonBar.FindByID(ID_COMBO_TIME_CHOSE);
  if (pTimeChose)
    return pTimeChose->GetCurSel();
  return 0;
}
//是否勾选正多边形
bool CMainFrame::RegularPolygon() const {
  CMFCRibbonCheckBox* pRegularPolygon =
      (CMFCRibbonCheckBox*)m_wndRibbonBar.FindByID(ID_REGULAR_POLYGON);
  if (pRegularPolygon)
    return pRegularPolygon->IsChecked();
  return false;
}
//正多边形边数
int CMainFrame::RegularEdges() const {
  CMFCRibbonComboBox* pRegularEdges =
      (CMFCRibbonComboBox*)m_wndRibbonBar.FindByID(ID_POLYGON_EDGES);
  if (pRegularEdges)
    return pRegularEdges->GetCurSel();
  return 0;
}
  //焦距距离
int CMainFrame::FocalDistance() const {
  CMFCRibbonEdit* pFocalDis =
      (CMFCRibbonEdit*)m_wndRibbonBar.FindByID(ID_3DFOCAL_DISTANCE);
  if (pFocalDis)
    return _ttoi(pFocalDis->GetEditText());
}

//材质选择
int CMainFrame::MaterialSelect() const {
  CMFCRibbonComboBox* pMs =
      (CMFCRibbonComboBox*)m_wndRibbonBar.FindByID(ID_COMBO_TEAPOT_MATERIAL);
  if (pMs)
    return pMs->GetCurSel();
  return 0;
}
...
COLORREF CMainFrame::FloodColor() const {
  CMFCRibbonColorButton* pBackColor =
      (CMFCRibbonColorButton*)m_wndRibbonBar.FindByID(ID_BACK_COLOR);
  if (pBackColor)
...
//根据激活的文档显示场景树
void CMainFrame::FillClassView() {
  m_wndClassView.FillClassView();
}


void CMainFrame::UnselectClassView()  //撤销场景树中的选择
{
  m_wndClassView.UnselectAll();
}

int CMainFrame::threeDStyle() const {
  CMFCRibbonComboBox* pPenStyle =
      (CMFCRibbonComboBox*)m_wndRibbonBar.FindByID(ID_DRAW_3DMODE);
  if (pPenStyle)
    return pPenStyle->GetCurSel();
  return 0;
 }