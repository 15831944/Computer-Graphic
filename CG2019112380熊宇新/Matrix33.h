...
class Matrix33 {
 public:
  ...
  // 拷贝构造
  Matrix33(const Matrix33& other) { operator=(other); }
  ...
  /// 将所有元素设为指定值
  Matrix33& fill(T val) {
    e(0, 0) = e(1, 0) = e(2, 0) = e(0, 1) = e(1, 1) = e(2, 1) = e(0, 2) =
        e(1, 2) = e(2, 2) = val;
    return *this;
  }
  ...
  // 重载赋值运算符
  Matrix33& operator=(T val) { return fill(val); }
  ...
  Matrix33& operator*=(const Matrix33& m) { return postMultiply(m); }
  ...
  //返回一个空矩阵
  static Matrix33 getNull() { return Matrix33().fill(0); }
  ...
  //返回一个单位矩阵
  static Matrix33 getIdentity() { return Matrix33(); }
  ...
  static Matrix33& multiply(Matrix33& out,const Matrix33& p,const Matrix33& q) {
    ...
    out.e(0, 0) =
        q.e(0, 0) * p.e(0, 0) + q.e(1, 0) * p.e(0, 1) + q.e(2, 0) * p.e(0, 2);
    out.e(0, 1) =
        q.e(0, 1) * p.e(0, 0) + q.e(1, 1) * p.e(0, 1) + q.e(2, 1) * p.e(0, 2);
    out.e(0, 2) =
        q.e(0, 2) * p.e(0, 0) + q.e(1, 2) * p.e(0, 1) + q.e(2, 2) * p.e(0, 2);

    out.e(1, 0) =
        q.e(0, 0) * p.e(1, 0) + q.e(1, 0) * p.e(1, 1) + q.e(2, 0) * p.e(1, 2);
    out.e(1, 1) =
        q.e(0, 1) * p.e(1, 0) + q.e(1, 1) * p.e(1, 1) + q.e(2, 1) * p.e(1, 2);
    out.e(1, 2) =
        q.e(0, 2) * p.e(1, 0) + q.e(1, 2) * p.e(1, 1) + q.e(2, 2) * p.e(1, 2);

    out.e(2, 0) =
        q.e(0, 0) * p.e(2, 0) + q.e(1, 0) * p.e(2, 1) + q.e(2, 0) * p.e(2, 2);
    out.e(2, 1) =
        q.e(0, 1) * p.e(2, 0) + q.e(1, 1) * p.e(2, 1) + q.e(2, 1) * p.e(2, 2);
    out.e(2, 2) =
        q.e(0, 2) * p.e(2, 0) + q.e(1, 2) * p.e(2, 1) + q.e(2, 2) * p.e(2, 2);
  ...
  Matrix33& preMultiply(const Matrix33& m) {
    ...
    return *this = multiply(t, m, *this);
      ...
      //任意直线的对称变化
  static Matrix33 getMirror(T degrees, const Vector2<T>& v) {
    return getTranslation(v) * getRotation(degrees) * getMirrorXAxis() *
           getRotation(-degrees) * getTranslation(-v);
  }
  //给定绕坐标原点旋转角度（单位：度）获取对应旋转矩阵（点坐标采用列向量形式）
  static Matrix33 getRotation(T degrees);

  //给定旋转角度和旋转中心，返回对应旋转矩阵（HTJ）
  static Matrix33 getRotation(T degrees, const Vector2<T>& v) {
    return getTranslation(v) * getRotation(degrees) * getTranslation(-v);
  }

  //返回一个从from旋转到to（都是过原点的直线）的矩阵(HTJ)
  static Matrix33& getRotation(const Vector3<T>& from, const Vector3<T>& to);

  //绕原点旋转指定角度（单位：度）（点坐标采用列向量形式，采用左乘旋转矩阵方式）
  Matrix33& rotate(T degrees) { return preMultiply(getRotation(degrees)); }

  //给定平移矢量，将传入的矩阵设为移矩阵并返回传入矩阵（左值）
  ...
  Matrix33& translate(T x, T y) { return preMultiply(getTranslation(x, y)); }
  ...
  //给定缩放系数、缩放参考点（不变点）、X方向（Y方向）返回缩放矩阵
  static Matrix33 getScaling(const Vector2<T>& c,
                             const Vector2<T>& ref,
                             const Vector2<T>& x) {
    Vector2 xAxis(1, 0);

    return getTranslation(ref) * getRotation(xAxis, x) * getScaling(c) *
           getRotation(x, xAxis) * getTranslation(-ref);
  }

  //按给定缩放系数缩放（比例）（点坐标采用列向量形式，采用左乘旋转矩阵方式）
  Matrix33& scale(T x, T y) { return preMultiply(getScaling(x, y)); }
  ...
  //关于x轴对称（列向量方式）
  Matrix33& mirrorXAxis() { return preMultiply(getMirrorX()); }

  //关于y轴对称（列向量方式）
  Matrix33& mirrorYAxis() { return preMultiply(getMirrorY()); }

  //关于原点对称（列向量方式）
  Matrix33& mirrorOrigin() { return preMultiply(getMirrorOrigin()); }

  //关于y = x线对称（列向量方式）
  Matrix33& mirrorYepX() { return preMultiply(getMirrorYepX()); }

  //关于y=-x轴对称（列向量方式）
  Matrix33& mirrorYenX() { return preMultiply(getMirrorYenX()); }
  ...
  // x轴方向错切
  Matrix33& shearXAxis(T shx) { return preMultiply(getShearXAxis(shx)); }

  // y轴方向错切
  Matrix33& shearYAxis(T shy) { return preMultiply(getShearYAxis(shy)); }

  //错切（指定x、y方向错切因子）
  Matrix33& shear(T shx, T shy) { return preMultiply(getShear(shx, shy)); }
...
T Matrix33<T>::getInverse(Matrix33<T>& dest) const {
  if (&dest == this) {
    ...
    if (det == 0)
      ...
      dest = Matrix33<T>(A, a13 * a32 - a33 * a12, a12 * a23 - a22 * a13, B,
                         a11 * a33 - a31 * a13, a13 * a21 - a23 * a11, C,
                         a12 * a31 - a32 * a11, a11 * a22 - a21 * a12) /
             det;
    return det;
  }
}
...
template <typename T>
Matrix33<T>& Matrix33<T>::getRotation(const Vector3<T>& from,
                                      const Vector3<T>& to) {
  Vector2<T> a, b;
  a = from;
  b = to;
